...
| Bypass-permissions session (`--dangerously-skip-permissions`, `bypassPermissions` mode, `CLAUDE_DANGEROUSLY_SKIP_PERMISSIONS=1`) | Hook-based refusals are silently disabled in this mode (per Anthropic GH issues #39523, #18846, #41615). Categorical refusal layer collapses. | Skill detects bypass-mode at session start (env vars + permission-state probe) and **refuses to operate** — emits a one-line message explaining un-punt requires the standard permission system. No sweeps, no captures, no edits. |
