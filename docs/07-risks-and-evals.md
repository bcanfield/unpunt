...
- **B8 *(promoted to elevated, weight 30%; MATERIALIZED May 2026 — mitigated via Decision #21 hooks)*: Skill `description` reliably auto-loads in real Claude Code sessions and survives auto-compaction.** The MVP "no hooks" decision originally rested on this. The golden-set eval runs via Claude Agent SDK and *seeds* the skill into the session — it doesn't test whether real Claude Code's description-match heuristic picks the skill up unprompted. Two concrete failure modes were documented in the Anthropic skills docs (April 2026):
  ...
  **Outcome (May 2026)**: B8 materialized in the punt-board × un-punt v0.1 dogfood. Probes 1, 2, 7 (description-match auto-load on sessions including fresh sessions): all FAILED on a well-formed ~1,100-char description well within the 1,536-char budget. Probes 4, 6, 8 (post-bootstrap silent capture + wrap-up suggestion): all FAILED — even when the skill was loaded via `/un-punt` slash-command bootstrap, the agent did not enforce capture/suggestion rules during normal Edit/Write/MultiEdit work or at textbook trigger phrases. **Mitigation per Decision #21**: SessionStart + PostToolUse + UserPromptSubmit hooks added at v0.2 MVP (the original "hooks become necessary" Phase 2 contingency). Hooks emit `additionalContext` per Q4a Sketch (ii) — structural pre-filter only, no content classification, preserving Decision 2. v0.2 re-dogfood (Q7 in `docs/v0.2-plan.md`) re-runs the failed probes; pass criterion = all 4 failed probes (1/2/7, 4, 6/8) now pass + cold-start regression-free + Sketch (ii) compliance ≥ 80%. If post-bootstrap compliance < 60%, escalate to Sketch (iv) `type: "prompt"` hooks per Q3b's deferred path (would need new decision-register entry superseding Decision 2 bullets 3 + 4).
...
| **C8** *(corrected May 2026)* | `--dangerously-skip-permissions` makes `permissions.deny` enforcement unreliable on categorical-refusal paths. Hooks fire normally (earlier "hooks silently disabled" claim was wrong). | medium | medium (was critical) | Skill operates normally; interpretive refusals + disposition prompt are load-bearing in bypass mode. v0.3 may add a `PreToolUse` refusal-floor hook for stronger bypass-mode safety per `docs/v0.3-roadmap.md` V03-10. |
