...
> - **Bypass mode (`--dangerously-skip-permissions`)**: hooks fire normally (corrected May 2026 — earlier "hooks silently disabled" claim was empirically wrong; see Decision 14 amendment for the citation re-reading). What IS reduced in bypass: `permissions.deny` enforcement of categorical-refusal paths. Skill body's interpretive refusals + disposition prompt remain load-bearing.
